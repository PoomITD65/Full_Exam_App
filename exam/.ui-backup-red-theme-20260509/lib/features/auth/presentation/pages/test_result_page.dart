    ...
    try {
      ...
      final rows = items.map<_ScoreRow>((e) {
        ...
        final score = (e['score'] as num?)?.toInt() ??
            ...
            0;
        final checked =
            (e['checked'] as bool?) ?? (e['Checked'] == 1) ?? false;
        final rYear =
            (e['rYear'] as num?)?.toInt() ?? int.tryParse('${e['rYear'] ?? ''}');
      ...
      setState(() {
        _subjectHeader =
            '${quiz['title'] ?? 'ไม่ระบุชื่อชุดข้อสอบ'} '
            '(${quiz['subjectNo'] ?? '-'}) ';
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'ตั้งค่าเฉลย',
            icon:
                const Icon(Icons.manage_search_rounded, color: Colors.white),
            onPressed: _openAnswerSettings,
          ),
        ],
      ),
                      ...
                      child: _BarItem(
                        ...
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (_) => const HomePage(token: '')), // Replace empty string with actual token
                          );
                        },
                      ),
                    ),
                        ...
                        child: InkResponse(
                          ...
                          child: Container(
                            ...
                            child: const Icon(Icons.qr_code_scanner_rounded,
                                color: Color(0xFF0E1320), size: 26),
                          ),
                        ),
                      ),
                    ),
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: const Row(
                children: [
                  _Cell('รหัสนักเรียน', flex: 20, bold: true, color: Colors.white),
                  _Cell('รายชื่อ',      flex: 32, bold: true, color: Colors.white),
                  _Cell('คะแนนที่ได้',  flex: 16, bold: true, center: true, color: Colors.white),
                  _Cell('ห้อง',         flex: 16, bold: true, center: true, color: Colors.white),
                  _Cell('สถานะ',        flex: 16, bold: true, center: true, color: Colors.white),
                ],
              ),
            ),
              ...
              child: rows.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        final statusText = r.checked ? 'ตรวจแล้ว' : 'ยังไม่ตรวจ';
                        final statusColor =
                            r.checked ? Colors.green[700] : Colors.redAccent;
...
class _AnswerSettingsPageState extends State<AnswerSettingsPage> {
  ...
  try {
    ...
    for (final r in rows) {
      ...
      if (no != null && no >= 1 && no <= answers.length &&
          ans != null && ans >= 1 && ans <= 5) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: kBg,
        title: const Text('ตั้งค่าเฉลย', style: TextStyle(color: Colors.white)),
        actions: [
          IconButton(
            tooltip: 'ล้างทั้งหมด',
      ...
      body: _loading
          ...
          : _error != null
              ? Center(child: Text('เกิดข้อผิดพลาด:\n$_error',
                                  ...
                                  children: [
                                    ...
                                    if (selected != null) ...[
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: _saving ? null : () => setState(() => answers[i] = null),
                                      )
                                    ]
                                  ],
                                ),
                        ...
                        SafeArea(
                          ...
                          child: SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              label: Text(_saving ? 'กำลังบันทึก...' : 'บันทึกเฉลย'),
