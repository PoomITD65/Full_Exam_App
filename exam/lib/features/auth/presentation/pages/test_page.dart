...
class _TestPageState extends State<TestPage> {
  ...
  Future<void> _load() async {
    try {
      ...
      _all = rows.map((e) {
        final dynamic rawPp =
            e["ppercent"] ?? e["passPercent"] ?? e["PPercent"];
        final double? d =
            (rawPp is num) ? rawPp.toDouble() : double.tryParse("$rawPp");
        final int passInt = (d ?? 60).round();

        final int id = int.tryParse("${e["id"] ?? e["quizId"] ?? 0}") ?? 0;
        final dynamic rawTerm = e["term"];
        final int term =
            (rawTerm is int) ? rawTerm : (int.tryParse("$rawTerm") ?? 0);
        final int year = (e["year"] is int)
            ? e["year"] as int
            : (int.tryParse("${e["year"]}") ?? 0);
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                            child:
                                _field('Year', controller: year, number: true)),
                      ],
                    ),
                    const SizedBox(height: 10),
                    ...
                    const SizedBox(height: 10),
                    _field('เกณฑ์ผ่าน (%) - ไม่กรอกได้',
                        controller: ppCtrl, number: true),
