...
// โหลดชุดข้อสอบที่ผู้ใช้นี้รับผิดชอบ
    ...
    try {
      ...
      for (final it in items) {
        final qid = int.tryParse(
                '${g(it, "QuizID") ?? g(it, "quizId") ?? g(it, "id")}') ??
            -1;
        if (qid <= 0) continue;
  ...
  Future<void> _initCamera() async {
    try {
      if (kIsWeb)
        return; // เนเธกเนเน€เธเธดเธ”เธเธฅเนเธญเธ live เธเธเน€เธงเนเธ เนเธซเนเนเธเนเธเธธเนเธกเธ–เนเธฒเธข/เธญเธฑเธเนเธซเธฅเธ”เนเธ—เธ
      ...
      builder: (_) => AlertDialog(
        ...
        content: Row(
          children: const [
            ...
            SizedBox(width: 10),
            Expanded(
                child: Text('กำลังอัปโหลดภาพและตรวจ...',
        ...
        Scaffold(
          ...
          appBar: AppBar(
            ...
            title: const Text('ตรวจข้อสอบ',
                style: TextStyle(color: kAccent, fontWeight: FontWeight.w900)),
                    ...
                    Material(
                      ...
                      child: InkWell(
                        ...
                        child: const SizedBox(
                          height: 32,
                          width: 36,
                          child: Icon(Icons.refresh, color: kSubtle, size: 20),
                        ),
                      ),
                    ),
                  ],
    ...
    if (!kIsWeb && cam != null) {
      return FutureBuilder(
        ...
        builder: (context, snap) {
          ...
          return const Center(
              child: Icon(Icons.center_focus_weak, size: 64, color: kSubtle));
        },
      );
    }
    // บนเว็บแสดง placeholder
    return const Center(
      child: Icon(Icons.camera_alt_rounded, size: 64, color: kSubtle),
    );
  }
}
